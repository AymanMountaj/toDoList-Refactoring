...
$(() =>{
    ...
    function stampaTodo(){
        ...
        let lista = $('#stampaLista').html('');
        let localLista = localStorage.getItem('listaTodo');
        if (localLista !== null) {
            todolist = JSON.parse(localLista);
        }
        $.each(todolist, function(i, el){
            $(lista).append(`<li class="list-group-item"> ${el.titolo} - ${el.testo}<span id="${i}" class="btn btn-sm btn-danger float-end rimuovi ">X</span></li>`);
        });
    }
